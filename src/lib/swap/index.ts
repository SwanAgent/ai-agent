import {
  ...
  async function getTransactionMessages(tokenIn: TokenMetadata, tokenOut: TokenMetadata, accountId: string, amountIn: string): Promise<TransformedTransaction[] | { error: string }> {
    ...
    const refSwapTransactionRes = await fetch(`https://ref-finance-agent-next.vercel.app/api/swap/${tokenIn.id}/${tokenOut.id}/${amountIn}`)
    const refSwapTransactions: RefSwapTransaction = await refSwapTransactionRes.json();
  
    console.log("refSwapTransactions", refSwapTransactions)
    if (isNearIn) {
      // wrap near
      refSwapTransactions.unshift(...transformTransactions([nearDepositTransaction(amountIn)], accountId));
    }
    ...
    if (!hasStorageIn) {
      refSwapTransactions.unshift(...transformTransactions([{
    ...
    //   }
    // }
    return refSwapTransactions.map((tx) => ({...tx, signerId: accountId}));
  }
