...
export type SubmitButtonState = {
  icon?: ReactElement<{ className?: string; [key: string]: any }>;
  title?: string;
  isLoading?: boolean;
  isDisabled?: boolean;
  onClick?: () => void;
...
export default function SubmitButton({
  ...
  return (
    <button
      className={cn(
        "relative flex min-h-[60px] w-full flex-row items-center justify-center gap-2.5 rounded-md bg-primary px-4 py-2.5 text-white transition-opacity disabled:opacity-50",
        className,
      )}
